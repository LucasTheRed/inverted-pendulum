...
class Ultra96IO:
	def __init__(self, params, encoder_pins=None, motor_pins=None, limit_pins=None):
		...
		off=int(float(self.__read_encoder_pin("in_steps0_raw", self.encoder_pins[0]))/self.encoder_scale)
		self.setEncoderOffset(0, -off)
...
#		print("Returning " + str(self.limit[index]) + " for switch " + str(index))
	...
	def read(self):
		...
		self.encoder[0] = (float(self.__read_encoder_pin("in_steps0_raw", self.encoder_pins[0])) / self.encoder_scale)*2
		self.encoder[1] = (float(self.__read_encoder_pin("in_steps0_raw", self.encoder_pins[1])) / self.encoder_scale)*2
...
#		print("Setting motor voltage: " + str(voltage))
		...
		self.__write_pwm_pin("duty_cycle", 0, int(min(abs(duty_cycle), self.params['motor_pwm_period']['val'])))
